...
import { cn, formatRating, formatViewCount, getGenreColors, parseRating } from '@/lib/utils';
              ...
              {parseRating(movie.averageRating) > 0 && (
                <div className="flex items-center gap-1">
                  ...
                  {movie.totalRatings > 0 && (
                    <span className="text-gray-500 hidden sm:inline">({movie.totalRatings} ratings)</span>
                  )}
                </div>
              )}
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            {(movie.director || movie.cast) && (
              <div className="space-y-3 sm:space-y-4">
                {movie.director && (
                  <div>
                    <h4 className="text-xs sm:text-sm font-semibold text-gray-400 mb-1">Director</h4>
                    <p className="text-white text-sm sm:text-base">{movie.director}</p>
                  </div>
                )}
                
                {movie.cast && (
                  <div>
                    <h4 className="text-xs sm:text-sm font-semibold text-gray-400 mb-1">Diễn viên</h4>
