...
export const moviesApi = {
  ...
  update: async (
    id: number,
    data: Partial<CreateMovieRequest>
  ): Promise<Movie> => {
  ...
  crawl: async (crawlUrl: string): Promise<Movie> => {
    const response = await api.post<Movie>('/movies/crawl', {
      crawlUrl,
    })
    return response.data
  },
...
export const ratingsApi = {
  ...
  getMovieAverage: async (
    movieId: number
  ): Promise<{ average: number; count: number }> => {
    const response = await api.get<{ average: number; count: number }>(
      `/ratings/movie/${movieId}/average`
    )
    return response.data
  },
...
export const watchlistApi = {
  ...
  check: async (
    movieId: number
  ): Promise<{ inWatchlist: boolean; item?: Watchlist }> => {
    const response = await api.get<{ inWatchlist: boolean; item?: Watchlist }>(
      `/watchlist/movie/${movieId}/check`
    )
    return response.data
  },
}
